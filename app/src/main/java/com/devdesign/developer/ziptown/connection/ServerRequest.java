package com.devdesign.developer.ziptown.connection;
...
import com.devdesign.developer.ziptown.models.forms.UpdateUser;
import com.devdesign.developer.ziptown.models.forms.UserLogin;
import com.devdesign.developer.ziptown.models.objectModels.Offers;
...
import com.devdesign.developer.ziptown.models.responses.GenericSuccessResponse;
import com.devdesign.developer.ziptown.models.responses.UserSignInAndLoginResponse;
import com.devdesign.developer.ziptown.models.forms.CreateCustomSearch;
...
public class ServerRequest extends AsyncTask<Map<String, Object>, Void, Object >{
    ...
    private Object[] createOfferSearch(Map<String, Object> map, RestTemplate restTemplate){
        ...
        if (response.length == 0){
            ...
            LandingPageActivity.zipCache.clearTable(LandingPageActivity.zipCache.OFFERS_SEARCH);
            for (int i = 0; i < response.length; i ++){
               LandingPageActivity.zipCache.addOffersSearch(LandingPageActivity.zipCache.toContentValues(response[i].ObjectToMap(response[i])));
    ...
    private Object[] createRequestSearch(Map<String, Object> map, RestTemplate restTemplate){
        ...
        if (response.length == 0){
            ...
            LandingPageActivity.zipCache.clearTable(LandingPageActivity.zipCache.OFFERS_SEARCH);
            for (int i = 0; i < response.length; i ++){
                LandingPageActivity.zipCache.addRequestsSearch(LandingPageActivity.zipCache.toContentValues(response[i].ObjectToMap(response[i])));
    ...
    private void cacheUserData(UserSignInAndLoginResponse response) {
        LandingPageActivity.zipCache.clearTable(ZipCache.USER_DATA);
        LandingPageActivity.zipCache.addUserData(LandingPageActivity.zipCache.toContentValues(response.getUserMap()));
    }

    private void cacheUserOffers(UserSignInAndLoginResponse response) {
        LandingPageActivity.zipCache.clearTable(ZipCache.USER_OFFERS);
        ...
        for(int i = 0; i < idList.size(); i++){
            ...
            LandingPageActivity.zipCache.addUserOffers(LandingPageActivity.zipCache.toContentValues(response.ObjectToMap(offersMap.get(idList.get(i).toString()))));
        }

    }
    private void cacheUserRequests(UserSignInAndLoginResponse response) {
        LandingPageActivity.zipCache.clearTable(ZipCache.USER_REQUESTS);
        ...
        for(int i = 0; i < idList.size(); i++){
            ...
            LandingPageActivity.zipCache.addUserRequests(LandingPageActivity.zipCache.toContentValues(response.ObjectToMap(requestsMap.get(idList.get(i).toString()))));
        }
        ...
        if (response.length == 0){
            ...
            if(responseAll.length == 0){
                ...
                if(map.get("postType").toString().equals("offers")){
                    LandingPageActivity.zipCache.clearTable(ZipCache.OFFERS);
                }else {
                    LandingPageActivity.zipCache.clearTable(ZipCache.REQUESTS);
                }
                for (int i = 0; i < responseAll.length; i ++){
                    ...
                    if(map.get("postType").toString().equals("offers")){
                        LandingPageActivity.zipCache.addOffers(LandingPageActivity.zipCache.toContentValues(responseAll[i].ObjectToMap(responseAll[i])));
                    }else {
                        LandingPageActivity.zipCache.addRequests(LandingPageActivity.zipCache.toContentValues(responseAll[i].ObjectToMap(responseAll[i])));
                    }
            ...
            for (int i = 0; i < response.length; i ++){
                ...
                if(map.get("postType").toString().equals("offers")){
                    LandingPageActivity.zipCache.addOffers(LandingPageActivity.zipCache.toContentValues(response[i].ObjectToMap(response[i])));
                }else {
                    LandingPageActivity.zipCache.addRequests(LandingPageActivity.zipCache.toContentValues(response[i].ObjectToMap(response[i])));
                }
